...
// bus_stop_demo.cpp
//
// (C) Copyright 2013 Daniel Koester
...
class gps_position
{
    ...
    void serialize(Archive & ar, const unsigned int /* file_version */){
        ar & BOOST_SERIALIZATION_NVP(degrees) & BOOST_SERIALIZATION_NVP(minutes) & BOOST_SERIALIZATION_NVP(seconds);
    }
public:
...
class bus_stop_corner : public bus_stop
{
    ...
    void serialize(Archive &ar, const unsigned int /*version*/)
    {
        ...
        ar & BOOST_SERIALIZATION_NVP(street1) & BOOST_SERIALIZATION_NVP(street2);
    }
...
class bus_stop_destination : public bus_stop
{
    ...
    void serialize(Archive &ar, const unsigned int /*version*/)
    {
        ar & BOOST_SERIALIZATION_BASE_OBJECT_NVP(bus_stop) & BOOST_SERIALIZATION_NVP(name);
    }
public:
...
boost::shared_ptr<boost::archive::hdf5_memory_buffer> buffer)
{
    // make an archive
...
void restore_schedule(bus_schedule &s,
boost::shared_ptr<boost::archive::hdf5_memory_buffer> buffer)
{
    // open the archive
    boost::archive::hdf5_iarchive ia(buffer);
...
int main()
{
    ...
    boost::shared_ptr<boost::archive::hdf5_memory_buffer> buffer
        = boost::make_shared<boost::archive::hdf5_memory_buffer>(1000000);

    // save the schedule
    save_schedule(original_schedule, buffer);

    // ... some time later
    // make  a new schedule
    bus_schedule new_schedule;

    restore_schedule(new_schedule, buffer);
